...
type Opts struct {
	...
	FamilyName    string `short:"F" long:"family" description:"Transaction Family name"`
...
func main() {
	...
	prefix := h.Hexdigest(opts.FamilyName)[:6]
	handler := h.NewHandler(prefix)
	handler.SetFamilyName(opts.FamilyName)
	handler.SetFamilyVersion(opts.FamilyVersion)
